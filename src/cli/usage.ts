/**
 * The complete `goaly help` text — every flag and subcommand, grouped by workflow. The config-file
 * drift test and the docs-sync check both treat `USAGE` as the user-facing flag contract, so a flag
 * that isn't documented here effectively doesn't exist.
 *
 * The text is kept as named TOPICS so `goaly help <topic>` can serve one section and the default
 * `goaly help` can stay short; `USAGE` is their concatenation, so the contract is unchanged.
 */
export type UsageTopic = {
  /** The name typed after `goaly help`. */
  readonly key: string;
  /** Short title for the topic index. */
  readonly title: string;
  /** The section text exactly as it appears in the full usage. */
...
Quick start (the LLM authors & checks everything, with Claude — just give it a goal):
  ...
  goaly --mode review "<goal>"   keep a human at every gate: approve the contract at Seal
...
function topic(key: string, title: string, text: string): UsageTopic {
  return { key, title, text };
}

export const USAGE_TOPICS: readonly UsageTopic[] = [
  topic('input', 'Goal / intent / rubric input', `Goal / intent / rubric input (choose ONE source per field):
  ...
  topic('verification', 'Verification', `Verification (--verify-cmd and --generate are mutually exclusive — passing both on the command line
is a usage error; --generate still overrides a verify-cmd inherited from a config file, loudly):
  ...
  topic('stuck', 'Stuck-detection tuning', `Stuck-detection tuning:
  ...
  topic('adversarial', 'Adversarial review', `Adversarial review (opt-in; a run without --adversarial is byte-for-byte unchanged):
  ...
  topic('satisfiability', 'Satisfiability critic', `Satisfiability critic (ON by default under --generate — the one review step that is):
  ...
  topic('dry-run', 'Contract dry run', `Contract dry run — the compile-time POSITIVE control (ON by default under --generate):
  ...
  topic('models', 'Model selection', `Model selection (all optional; default = each tool's own default):
  ...
  topic('goaly-code', 'goaly-code harness / OpenAI-compatible endpoint', `goaly-code harness / OpenAI-compatible endpoint (--harness goaly-code, --llm-provider openai):
  ...
  topic('seal', 'Seal', `Seal (contract approval — the review point before any execution):
  ...
  --preset <name>|none        apply a NAMED preset — a flag bundle defined under "presets" in a
                              ...
                              it yours. 'default' is truly the default: a run that chooses NEITHER
                              a preset NOR a mode gets it implicitly — but only as the weakest
                              tier, filling gaps (any config-file key or explicit flag wins, and
                              it is announced with its off-switch every run). --preset none (or a
                              persisted "preset": "none") opts out entirely. An explicitly chosen
                              preset expands at parse time exactly like --mode (nothing invisible
                              reaches the loop; every expansion is logged), and may itself set
                              "mode". Layering: implied default < config base keys < chosen preset
                              < --mode < explicit CLI flags. Also settable as a top-level "preset"
                              in .goalyrc to pick the preset applied on every run. Unknown names
                              fail closed listing what is defined (goaly config presets shows the
                              same).
  ...
  topic('timeouts', 'Per-step timeouts', `Per-step timeouts (subprocess kill-timeouts in milliseconds; all optional, pure wiring):
  ...
  --preset <name>, or persist "preset": "<name>" to apply it by default. Example:
    { "presets": { "quick": { "mode": "review", "max-iterations": 3 },
      ...
      "preset": "quick" }
  A built-in 'default' preset ({ "mode": "hands-off" }) always exists and applies IMPLICITLY
  (gap-filling only) when a run chooses neither a preset nor a mode; "preset": "none" opts a
  whole tree out. A config layer that redefines a preset name (built-in or not) replaces it
  wholesale (no body merging).`),
  topic('spend', 'Per-run spend report', `Per-run spend report (printed at the end of every run; stored in the run log):
  ...
  topic('diagnostics', 'Diagnostics', `Diagnostics (leveled, structured logging — separate from the write-ahead run log):
  ...
  topic('resume', 'Resume, steer & extend', `Resume, steer & extend (operator control over ONE run — the frozen contract never changes):
  ...
  topic('follow-up', 'Follow-up after a run ends', `Follow-up after a run ends (build on a finished run — keeps every invariant by construction):
  ...
  topic('runs', 'Run history & inspection', `Run history & inspection (read-only — pure replay of the write-ahead run log, no re-running):
  ...
  --workspace-mode git|file|auto
                            ...
                            otherwise.`),
  topic('worktrees', 'Worktrees', `Worktrees (run on an isolated copy of the repo; merge back with plain git):
  ...
  topic('ui', 'Web UI', `Web UI (observe, start & steer runs from the browser):
  ...
  topic('onboarding', 'Onboarding', `Onboarding (first-time setup):
...
export const USAGE = [USAGE_HEAD, ...USAGE_TOPICS.map((t) => t.text)].join('\n\n');
