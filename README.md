...
```
COMPILE_VERIFIER → [Gate A: approve / revise / reject] → [setup + pre-flight] → loop {
...
- **One-time setup + pre-flight, before the first agent turn.** After Gate A and before iteration 1,
  ...
  contract (shown at Gate A) so it can't drift. A plain `--verify-cmd` run with no setup skips this phase.
